...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated PyTools loader file *)


Unprotect[PyTools`PackageScope`Private`$TopLevelLoad];
PyTools`PackageScope`Private`$TopLevelLoad=MemberQ[$ContextPath, "Global`"];


BeginPackage["PyTools`"];


ClearAll[PyTools];
PyTools::usage="PyTools is a manager head for the PyTools package";
...
PyTools["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
PyTools["Contexts"]:=$PackageContexts;
$PackageContexts=
	{
		"PyTools`",
		"PyTools`PackageScope`Private`",
		"PyTools`PackageScope`Package`"
		};
$PackageDeclared=
...
PyTools["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
PyTools["PackageScopedSymbols"]:=$PackageScopedSymbols;
$PackageScopedSymbols={};
PyTools["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
	Merge[
		{
			With[
...
PyTools["FileContexts"]:=$PackageFileContexts;
...
PyTools["DeclaredPackages"]:=$DeclaredPackages;
...
PyTools["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
	With[{s=PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s,PyTools],"`"]
		];
...
PyTools::nodep="Couldn't load dependency `` of type ``";
PyTools::nodup="Couldn't update dependency `` of type ``";
	...
	Block[{retcode, site, path, file, tmp},
		...
		site=
			...
			If[retcode<300,
				...
				DeleteFile[tmp];
				file,
				Message[PyTools::nodep, dep, "Package"];
				DeleteFile[tmp];
				$Failed
				]
			];
		...
		Monitor[
			MapThread[
				Check[
					...
					Message[PyTools::nodep, #2, "Paclet"];
					$Failed
					]&,
				{
					pacs,
			...
			Monitor[
				MapThread[
					Check[
						...
						Message[PyTools::nodup, #2, "Paclet"];
						$Failed
						]&,
					{
						StringDelete[deps, "`"],
...
PackageCatchMessage[
	...
	Replace[
		Thread[Map[Hold[MessageName[PyTools, #]]&, msg], Hold],
		{
			{}:>
				Check[expr, 
...
PackageFailureException[
	...
	(
		Set[MessageName[PyTools, tag], body];
		PackageFailureException[
			MessageName[PyTools, tag],
			body,
			ops
...
PackageAttachMainAutocomplete[]:=
	PackageAddAutocompletions[PyTools, 
		Table[
			Replace[{}->None]@
				Cases[
					DownValues[PyTools],
...
If[
	(Clear@PyTools`PackageScope`Private`$loadAbort;!#)&@
		PyTools`PackageScope`Private`$loadAbort,
	Unprotect[PyTools`PackageScope`Private`$LoadCompleted];
	PyTools`PackageScope`Private`$LoadCompleted=True;
	PyTools`PackageScope`Private`PackagePostProcessContextPathReassign[]
	]